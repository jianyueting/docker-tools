...
for i in $(seq 1 ${num});do
    ...
    docker run -d --rm --net=spark -v ${base}/data-${i}:/data --name spark-datanode${i} --hostname spark-datanode${i} jm-hadoop-spark &>/dev/null
done
...
docker run --rm -d --net=spark -v ${base}/name:/data \
    ...
    -p 19888:19888 \
    --name spark-namenode --hostname spark-namenode jm-hadoop-spark &>/dev/null
