...
if [ ${HOSTNAME} = "mysql-1" ];then
    ...
    echo  "mysql -u root -e \"change master to master_host='mysql-1', master_user='replication', master_password='"${passwd}"', master_log_file='"${file}"', master_log_pos="${pos}";\"" > /data/commond.line
