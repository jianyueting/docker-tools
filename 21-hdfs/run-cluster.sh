...
for i in $(seq 1 ${num});do
    ...
    docker run -d --rm --net=hdfs -v ${base}/data-${i}:/data --name hdfs-datanode${i} --hostname hdfs-datanode${i} jm-hdfs &>/dev/null
done
...
docker run --rm -d --net=hdfs -v ${base}/name:/data \
    ...
    -p 19888:19888 \
    --name hdfs-namenode --hostname hdfs-namenode jm-hdfs &>/dev/null
